...
import WatchLaterBorderOutlinedIcon from "@mui/icons-material/WatchLater";
...
import { addWatchLater, removeWatchLater } from "../redux/features/userSlice";
...
const MediaDetail = () => {
  ...
  const { user, listFavorites, listWatchLater } = useSelector((state) => state.user);
  ...
  const onWatchLaterClick = async () => {
    ...
    const { response, err } = await watchlaterApi.add(body);

    setOnRequest(false);

    if (err) toast.error(err.message);
    if (response) {
      dispatch(addWatchLater(response));
      setIsWatchLater(true);
      toast.success("Add Watchlater success");
    }
  };

  const onRemoveWatchLater = async () => {
    if (onRequest) return;
    setOnRequest(true);

    const watchlater = listWatchLater.find(e => e.mediaId.toString() === media.id.toString());

    const { response, err } = await watchlaterApi.remove({ watchlaterId: watchlater.id });

    setOnRequest(false);

    if (err) toast.error(err.message);
    if (response) {
      dispatch(removeWatchLater(watchlater));
      setIsWatchLater(false);
      toast.success("Remove Watchlater success");
    }
  };
                ...
                <Stack spacing={5}>
                  ...
                  <Stack direction="row" spacing={1}>
                    ...
                    <LoadingButton
                      ...
                      size="large"
                      startIcon={isFavorite ? <WatchLaterIcon /> : <WatchLaterBorderOutlinedIcon />}
                      loadingPosition="start"
                      loading={onRequest}
